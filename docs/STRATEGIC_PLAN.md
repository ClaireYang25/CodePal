...
### **阶段二：实现主动感知 (In Progress)**

**目标**: 让插件能“猜到”用户需要 OTP，并主动进入戒备状态，为最终的 Gmail API 架构做准备。

-   ✅ **行动 2.1：创建全局 OTP 输入框监听器**
    -   **任务**: 创建新的轻量 Content Script，注入至常见站点，检测具有 `autocomplete="one-time-code"` 或包含 OTP 关键词的输入框；一旦触发，向 Service Worker 发送高警戒信号。
    -   **现状**: 已完成。`otp-intent-listener.js` 会在全站范围内捕获用户与验证码输入框的交互，Service Worker 记录 `highAlertUntil` 和最近的意图区域，为后续动态轮询与通知打下基础。

-   🟡 **行动 2.2：实现动态轮询频率 (与阶段三联动)**
    -   **任务**: （仅在引入 Gmail API 后）当 Service Worker 收到“高度戒备”信号后，将后台轮询的频率从 30 秒动态调整为 5 秒，以实现更快的响应。

> 🔄 **阶段二进展**：已具备稳定的 OTP 意图监听机制。下一步是当高警戒模式激活时，配合 Gmail API 动态拉高轮询频率，并探索在无 Gmail 标签页情况下的 fallback 流程。
...
---
**当前结论**: 阶段一目标已全面完成；阶段二的第一步（全局 OTP 意图监听）已经落地。接下来专注于与 Gmail API 的衔接，利用高警戒信号动态调整轮询，逐步逼近真正的全后台体验。
